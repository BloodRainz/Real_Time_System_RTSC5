...
void reader(App *self, int c) {
	...
	switch(c)
	{
		...
		case 'D' : ;  // remove the semicolon if error araises -> ; is the bandage to error on WCET
			...
			int volumeTG = SYNC(&toneGenerator, getVolumeTG, 0);
			int prevVolueTG = SYNC(&toneGenerator, getPrevVolumeTG, 0);
			...
			snprintf(write_buf, 200, "Deadline from musicPlay %ld \n", WCETDeadline);
			SCI_WRITE(&sci0, write_buf);
			
			snprintf(write_buf, 200, "Volume from toneGenerator: %d \n", volumeTG);
			SCI_WRITE(&sci0, write_buf);
			
			snprintf(write_buf, 200, "Prev volume from toneGenerator %d \n", prevVolueTG);
			SCI_WRITE(&sci0, write_buf);

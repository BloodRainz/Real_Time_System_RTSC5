...
MusicPlayerObj musicPlayer = initMusicPlayer();
	...
	switch(c)
	{
		...
		case 'K':
			...
			else
			{
				...
				snprintf(write_buf, 200, "Current key is: %d", SYNC(&toneGenerator, getKey, NULL));
				SCI_WRITE(&sci0, write_buf);
			break;
			}
		break;
		...
		case 'H':
			...
			self->current_key = SYNC(&toneGenerator, getKey, NULL);
		...
		case 'G':
			...
			{
				self->current_key = SYNC(&toneGenerator, setKey, decr_key);
				snprintf(write_buf, 200, "New key: %d \n", self->current_key);
				SCI_WRITE(&sci0, write_buf);
				break;
			}
		...
		case 'T':
			...
			if (self->current_tempo == 0)
			{
				...
				snprintf(write_buf, 200, "Current tempo: %d\n", SYNC(&toneGenerator, getTempo, NULL));
				SCI_WRITE(&sci0, write_buf);
				break;
			}
	
			// If SYNC setTempo is successful, print new tempo
			if(SYNC(&toneGenerator, setTempo, self->current_tempo) == self->current_tempo)
			{
			...
			else
			{
				...
				snprintf(write_buf, 200, "Current tempo: %d", SYNC(&toneGenerator, getTempo, NULL));
				SCI_WRITE(&sci0, write_buf);
			break;
			}
		break;
		...
		case 'E':
			...
			self->current_tempo = SYNC(&toneGenerator, getTempo, NULL);
			...
			{
				self->current_tempo = SYNC(&toneGenerator, setTempo, incr_tempo);
				snprintf(write_buf, 200, "New tempo: %d \n", self->current_tempo);
				SCI_WRITE(&sci0, write_buf);
				break;
			}
		...
		case 'W':
			...
			{
				self->current_tempo = SYNC(&toneGenerator, setTempo, decr_tempo);
				snprintf(write_buf, 200, "New tempo: %d \n", self->current_tempo);
				SCI_WRITE(&sci0, write_buf);
				break;
			}
		...
		case 'D' : 
			...
			long WCETDeadline = SYNC(&toneGenerator, setDeadline, 0);

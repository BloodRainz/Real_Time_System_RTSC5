 ...
 * 2. Find a better way to understand the timing requirements of the system.
...
typedef struct {
	...
	int user_mute;
	int CANMode;				// Conductor/Musician mode
	CANMsg transmit_msg[CAN_BUFSIZE];		// CAN Message to transmit
} App;

App app = { initObject(), 0, 'X', {0}, 0, 0, 0 , 0};
...
void receiver(App *self, int unused) {
	...
	self->CANMode = SYNC(&canComms, getCANState, 0);
...
void reader(App *self, int c) {
	...
	switch(c)
	{
		...
		case 'M':
			...
			// Set length of message to 0: no values other than
			// a msgId is being sent.
			self->transmit_msg->length = 0;
			
			
			self->count = 0;
			
			// Assign msgId for mute functions
			...
			if(self->CANMode == 0)
			{
				if(SYNC(&toneGenerator, set_user_mute, self->user_mute) == 1)
				{
		...
		///////////////////////////////////////////////////////////////////////////////////////
		//
		// CAN Bus: switch between conductor and musician mode
		case 'P':
...
void startApp(App *self, int arg) {
	...
	SCI_WRITE(&sci0, "Press S to start and stop the player.\n");
	SCI_WRITE(&sci0, "//////////////////////////////////////////////////////////////////\n");
	...
	SCI_WRITE(&sci0, "/////////////////////////////////////////////////////////////////\n");
	SCI_WRITE(&sci0, "CAN INSTRUCTIONS: \n");
	SCI_WRITE(&sci0, "Press P to change Conductor/Musician states\n");
	SCI_WRITE(&sci0, "/////////////////////////////////////////////////////////////////\n");
    msg.msgId = 40;
    msg.nodeId = 1;
    msg.length = 6;
